...
public class ProvinceView extends AbstractView implements Component {
	...
	public ProvinceView(float x, float y) {
		...
		viewInfrastructure = bb.text("Infrastructure")
				.x((x + (WIDTH / 2)) - 64).instantiate();
		viewInfrastructure.addButtonListener((button) -> {
			currentView = ProvinceDataView.INFRASTRUCTURE;
		});

		viewOrganisations = bb.text("Organisations")
				.x((x + WIDTH) - TEXTX - 128).instantiate();
		...
		listDemographics = new ButtonList(x + (TEXTX * 2), y + (TEXTY * 5),
				128, 3, null);
	...
	public void internalRender(GameContainer container, StateBasedGame game,
		...
		graphics.drawString(prov.getData().getName() + " (" + prov.getId()
				+ ")", x + TEXTX, y + TEXTY);
		graphics.drawString(
				"Population: " + prov.getData().getPopulationSize(), x
						+ (TEXTX * 2), y + (TEXTY * 2));

		String resourceName = prov.getData().getResource().getName();
		graphics.drawString("Resource: " + resourceName, x + (TEXTX * 2), y
				+ (TEXTY * 3));
		graphics.drawString("Terrain: " + prov.getTerrainType(), x
				+ (TEXTX * 2), y + (TEXTY * 4));
		...
		if (prov.getData().getPopulation() != null) {
			...
			case INFRASTRUCTURE:
				...
				graphics.drawString("Railroad: " + infra.getRailroadLevel(),
						TEXTX * 2, y + (TEXTY * counter));
				counter += 1;
				graphics.drawString("Roadways: " + infra.getRoadLevel(),
						TEXTX * 2, y + (TEXTY * counter));
				counter += 1;
				graphics.drawString("Sea Port: " + infra.getPortLevel(),
						TEXTX * 2, y + (TEXTY * counter));
				break;

			case ORGANISATIONS:
				List<Organisation> organisations = prov.getData()
						.getOrganisations();
				if (organisations != null) {
					for (int i = 0; i < organisations.size(); i++) {
						Organisation item = organisations.get(i);
						graphics.drawString(item.getName(), TEXTX * 2, y
								+ (TEXTY * counter));
						counter += 1;
					}
				}
	...
	public void internalUpdate(GameContainer container, StateBasedGame game,
		...
		if (getCurrentProvince() != null) {
			if (getCurrentProvince().getData() != null) {
		...
		if (currentView == ProvinceDataView.DEMOGRAPHICS) {
			Province prov = getCurrentProvince();
			if ((prov != null) && prov.getData() != null && (prov.getData().getPopulation() != null)) {
				listDemographics.setData(getCurrentProvince().getData()
						.getPopulation().getSubPopulationString());
	...
	public static enum ProvinceDataView {
		DEMOGRAPHICS, INFRASTRUCTURE, ORGANISATIONS
	}
}
