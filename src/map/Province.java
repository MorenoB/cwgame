...
public class Province {
	...
	public ResourceTable getLocalResources() {
		...
		for (SubPopulation subPop : getData().getPopulation()
				.getSubPopulations()) {
	...
	public boolean isWater() {
		return (terrainType == TERRAIN_INLAND_SEA)
	...
	public void render(float xOffset, float yOffset, float zoom) {
		...
		image.startUse();
		image.drawEmbedded(x, y, image.getWidth() * zoom, image.getHeight()
				* zoom);
		image.endUse();
	}
	...
	public void testGenTexture() {
		/*if ((terrainType == TERRAIN_INLAND_SEA)
			...
			final ImageBuffer buf = new ImageBuffer((maxX - minX) + 1,
					(maxY - minY) + 1);
