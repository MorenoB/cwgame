...
public class DiplomacyView extends AbstractView implements Component {
	...
	public void internalRender(GameContainer container, StateBasedGame game, Graphics graphics) {
		...
		if(country != null) {
			...
			graphics.drawString(country.getData().getName(), getX() + TEXTX, getY() + TEXTY);
			graphics.drawString("Population: " + country.getPopulation(), getX() + (TEXTX * 2), getY()
					+ (TEXTY * 2));
			...
			if(country == GameContext.playerCountry) {
				...
				for(int i = 0; i < resourceAmounts.length; i++) {
					...
					graphics.drawString(actualResource.getName() + ": " + shortenedValue, getX()
							+ (TEXTX * 2), getY() + (TEXTY * ymod));
					ymod++;
				}
			} else {
