package gui.screens;

import org.newdawn.slick.GameContainer;
import org.newdawn.slick.Graphics;
import org.newdawn.slick.state.StateBasedGame;

public class PopulationScreen extends AbstractScreen {

	@Override
	protected void internalEnter(GameContainer gc, StateBasedGame sbg) {
	}

	@Override
	protected void internalInit(GameContainer gc, StateBasedGame sbg) {
	}

	@Override
	protected void internalLeave(GameContainer gc, StateBasedGame sbg) {
	}

	@Override
	protected void internalRender(GameContainer gc, StateBasedGame sbg,
			Graphics graphics) {
	}

	@Override
	protected void internalUpdate(GameContainer gc, StateBasedGame sbg,
			int delta) {
	}

}
